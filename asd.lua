...
function NullUI:ApplyColorTheme(primary, secondary)
	local nextTheme = {}
	if primary ~= nil then nextTheme.Accent = primary end
	if secondary ~= nil then nextTheme.Secondary = secondary end
	if nextTheme.Accent ~= nil or nextTheme.Secondary ~= nil then
...
NullUI.Theme = {
	...
	Secondary      = Color3.fromRGB(90, 90, 255),
...
function NullUI:ApplyTheme(theme, silent)
	if type(theme) ~= "table" then return self.Theme end
	for key, value in pairs(theme) do
		if self.Theme[key] ~= nil then
			self.Theme[key] = value
		end
	end
	if not silent and self._Windows then
		for _, window in ipairs(self._Windows) do
			if window and type(window.ApplyTheme) == "function" then
				pcall(window.ApplyTheme, window, self.Theme)
			end
		end
	end
	return self.Theme
...
function NullUI:SetDarkMode(enabled)
	local dark = enabled ~= false
	local accent = self.Theme.Accent or Color3.fromRGB(255, 255, 255)
	local secondary = self.Theme.Secondary or Color3.fromRGB(90, 90, 255)
	local palette = dark and {
		...
		Text       = Color3.fromRGB(19, 22, 29),
		TextDim    = Color3.fromRGB(92, 96, 104),
		Accent     = accent,
		Secondary  = secondary,
		Danger     = Color3.fromRGB(194, 79, 79),
	}
	return self:ApplyTheme(palette)
end
...
function NullUI:BindAutoSaveConfig(name, opts)
	opts = opts or {}
	local configName = tostring(name or "config")
	...
	local function saveNow()
		local ok, err = self:SaveConfig(configName, {
			Description = opts.Description or "Auto-saved UI settings",
			Tags = opts.Tags or { "ui", "autosave" },
		})
		if not ok then
			if opts.OnError then pcall(opts.OnError, err) end
		end
	end
	...
	for flag, api in pairs(self.Flags) do
		if api and type(api.OnChanged) == "function" then
			local conn = api:OnChanged(function()
				scheduleSave()
			end)
